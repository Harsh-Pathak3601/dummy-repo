...
export const BentoGridItem = ({
  ...
  const launchTerminal = () => {
    ...
    setTerminalHistory([
      ...
      { type: 'resp', text: "Welcome back, Admin. Type 'help' to see available commands." },
    ]);
  };
        ...
        newEntries.push({ 
          ...
          text: (
            <div className="flex flex-col gap-1 mt-1 font-mono">
              ...
              <a href="https://leetcode.com/Pathak" target="_blank" rel="noreferrer" className="text-emerald-400 hover:text-white transition-colors">💻 LeetCode: Pathak</a>
  ...
  return (
    ...
    >
      ...
      {isTerminalOpen && (
        <div className="fixed inset-0 z-[100] bg-[#04071d] font-mono p-4 md:p-10 flex flex-col animate-in fade-in zoom-in duration-300">
          <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.1)_50%),linear-gradient(90deg,rgba(168,85,247,0.02),rgba(0,255,0,0.01),rgba(0,0,118,0.02))] z-[101] bg-[length:100%_2px,3px_100%]" />
          <div className="flex justify-between items-center border-b border-purple-500/30 pb-4 mb-4 font-sans">
            <span className="text-purple-400 flex items-center gap-2 font-bold tracking-wider text-xs md:text-base">
              <span className="w-2 h-2 bg-purple-50 rounded-full animate-ping" />
              HARSH_PERSONAL_TERMINAL
            </span>
            <button onClick={() => setIsTerminalOpen(false)} className="text-[#C1C2D3] hover:text-white transition-colors relative z-[102] text-xl">
              <ImCancelCircle />
            </button>
          </div>
          <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-3 custom-scrollbar pr-4 text-xs md:text-base">
            {terminalHistory.map((line, i) => (
              <div key={i} className={cn(line.type === 'cmd' ? "text-purple-400 font-bold" : line.type === 'sys' ? "text-blue-300 opacity-60 italic font-sans" : "text-emerald-400")}>
                {line.type === 'cmd' && "➜ admin@harsh:~$ "} {line.text}
              </div>
            ))}
          </div>
          <form onSubmit={handleCommand} className="mt-4 flex items-center gap-2 border-t border-purple-500/20 pt-4 font-sans shrink-0">
            <span className="text-purple-500 shrink-0 font-bold text-xs md:text-base font-mono">➜ admin@harsh:~$</span>
            <input
              autoFocus 
              value={terminalInput}
              onChange={(e) => setTerminalInput(e.target.value)}
              className="bg-transparent border-none outline-none text-emerald-400 w-full caret-purple-500 font-mono text-xs md:text-base"
      ...
      <div className="h-full">
        ...
        {/* IDs 1, 2, 3, 4, 6 (Shared Title Styling) */}
        {(id === 1 || id === 2 || id === 3 || id === 4 || id === 6) && (
          <div className={cn(titleClassName, "group-hover/bento:translate-x-2 transition duration-300 relative h-full flex flex-col px-5 p-5 lg:p-10")}>
            ...
            {/* ID 6 BODY WITH INCREASED WIDTH & CENTERING */}
            {id === 6 ? (
              <div 
              ...
              >
                ...
                {/* TERMINAL BOX: WIDTH INCREASED */}
                <div className="w-full max-w-full md:max-w-full bg-black/60 border border-white/10 rounded-xl p-3 md:p-6 font-mono text-left backdrop-blur-md shadow-2xl transform group-hover/terminal:-translate-y-2 transition-transform duration-500 group-hover/terminal:border-purple-500/40 group-hover/terminal:shadow-purple-500/10 mx-auto">
                ...
                <div className="relative px-5 py-1.5 md:px-10 md:py-3 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-400 text-[10px] md:text-sm font-mono font-bold group-hover/terminal:bg-purple-500 group-hover/terminal:text-white transition-all duration-300 w-full md:w-auto text-center">
      ...
      <style jsx>{`
        ...
        .animate-skill-scroll:hover { animation-play-state: paused; }
